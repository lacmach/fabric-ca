...
func (ca *CA) initKeyMaterial(renew bool) error {
	...
	if !renew {
		...
		if certFileExists {
			log.Info("=====lib/ca.go:249===== certFile : ", certFile)
			log.Info("=====lib/ca.go:250===== ca.csp : ", ca.csp)
			_, _, _, err = util.GetSignerFromCertFile(certFile, ca.csp)
			log.Infof("The key is stored by BCCSP provider '%s'", ca.Config.CSP)
			if err != nil {
...
func (ca *CA) initEnrollmentSigner() (err error) {
	...
	// log.Info("====lib/ca.go:792==== ca.csp :%s ", ca.csp)
	log.Info("====lib/ca.go:792==== c.CA :%s ", c.CA.Keyfile)
