...
func getBCCSPKeyOpts(kr csr.KeyRequest, ephemeral bool) (opts bccsp.KeyGenOpts, err error) {
	...
	log.Debugf("=====util/csp.go:127======generate key from request: algo=%s, size=%d", kr.Algo(), kr.Size())
	...
	case "gmsm2", "sm2": //可能是传入了sm2==============================
		return &bccsp.GMSM2KeyGenOpts{Temporary: ephemeral}, nil
	default:
		return nil, errors.Errorf("=====util/csp.go:157======Invalid algorithm: %s", kr.Algo())
	}
}
...
func GetSignerFromCert(cert *x509.Certificate, csp bccsp.BCCSP) (bccsp.Key, crypto.Signer, error) {
	...
	log.Infof("====util/csp.go:166==== begin csp.KeyImport,cert.PublicKey is %T  csp:%T", cert.PublicKey, csp)
	switch cert.PublicKey.(type) {
	case *sm2.PublicKey:
		log.Infof("====util/csp.go:169==== cert is sm2 puk")
	default:
		log.Infof("====util/csp.go:171==== cert is default puk")
	}
	...
	log.Info("xxxx end GetSignerFromCert successful")
...
func GetSignerFromCertFile(certFile string, csp bccsp.BCCSP) (bccsp.Key, crypto.Signer, *x509.Certificate, error) {
	...
	fmt.Println("====util/csp.go:263====certFile++++++++++", certFile)
	//var newCert =&x509.Certificate{}
	if err != nil || cert == nil {
		log.Infof("====util/csp.go:266==== error = %s,Maybe it is a gm cert!", err.Error())
	...
	log.Infof("====util/csp.go:275==== Key = %T error = %v", key, err)
...
func BCCSPKeyRequestGenerate(req *csr.CertificateRequest, myCSP bccsp.BCCSP) (bccsp.Key, crypto.Signer, error) {
	log.Infof("======util/csp.go:268======generating key: %+v", req.KeyRequest)
